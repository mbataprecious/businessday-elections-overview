...
function extensions(parentClass) {
  return {
    ...
    removeLayer: function (layer) {
      let bboxs = this._cachedRelativeBoxes[layer._leaflet_id];
      for (let i = 0, n = bboxs.length; i < n; i++) {
        let bbox = arrToBBox(bboxs[i]);
        this._rbush.remove(bbox);
      }
    ...
    _maybeAddLayerToRBush: function (layer) {
      ...
      boxes = this._positionBoxes(
        this._map.latLngToLayerPoint(layer.getLatLng()),
        boxes
      );

      var collision = false;
      for (var i = 0; i < boxes.length && !collision; i++) {
        collision = bush.search(arrToBBox(boxes[i])).length > 0;
      }
